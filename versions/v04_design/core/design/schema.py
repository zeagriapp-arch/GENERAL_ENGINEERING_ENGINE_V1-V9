...
class Design(BaseModel):
    ...
    operating_conditions: dict[str, Parameter] = Field(
        default_factory=dict, description="Condiciones de operación (Phase 4, sección Design Representation)"
    )
    manufacturing_constraints: list[Constraint] = Field(
        default_factory=list, description="Restricciones de manufactura, separadas de las físicas/operativas"
    )
    provenance: list[str] = Field(
        default_factory=list, description="ids de Source/Experiment que informaron este diseño"
    )
